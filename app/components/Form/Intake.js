...
const Intake = () => {
  ...
  const sendTextAndNumber = (element, key) => {
    ...
    if (intakeCompleteStep > element.step) {
      return (
        <CheckCircleIcon className="bg-white text-primary rounded-full w-full h-full " />
      );
    }
    return key + 1;
  };
                ...
                <ol className="fixed">
                  {headings.map((element, key) => (
                    ...
                    >
                      ...
                      <h3 className="flex items-center font-bold h-[30px] m-0 text-xs leading-tight">
                        {element.text}
                      </h3>
                    </li>
                  ))}
