...
const NewSidebar = ({ children }) => {
  ...
  const handleLogout = () => {
    ...
    for (let i = 0; i < cookies.length; i++) {
        ...
        // Check if the cookie name is not "visit"
        if (cookieName !== "visit") {
  ...
  const sendSideBarDetails = (element, key) => {
    if (element.list.length > 0) {
      return excludeMenuArrayList?.includes(element.name) &&
        (stateBots?.botData?.data?.bots?.length == 0 ||
          stateBots?.botData?.data?.bots?.length == undefined) ? (
        ""
      ) : (
        <li
          key={key}
          className={`pt-1 w-full rounded-lg ${
            pathname === element.href && ""
          }`}
        >
          {!collaps && (
            <p className="pl-2 text-white font-semibold text-xs mt-1">
              {element?.name}
            </p>
          )}
              ...
              <>
                {ele.isLink === true ? (
                  <div className="rounded-lg cursor-pointer my-2">
                    <div
                      className={`flex items-center hover:bg-[#323B41]  rounded-t-lg p-2 ${
                    ...
                    >
                      ...
                      {!collaps && (
                        <span
                          className={`flex justify-between w-full ml-3 whitespace-nowrap text-[13px] font-normal transition-all duration-300 ease-in-out`}
                        >
                          {ele.name}
                        </span>
                      )}
                    ...
                    {pathname.includes(ele.href) && (
                      <div className={`${!collaps && "p-2"} bg-[#232D32]`}>
                        {ele.list.map((item, indexItem) => (
                          <li
                            key={indexItem}
                            className={`mb-1 px-2 hover:bg-sidebar-hover hover:text-white w-full rounded-lg ${
                              pathname === item.href
                  ...
                  <li
                    key={key}
                    className={`mb-1 hover:bg-sidebar-hover hover:text-white w-full rounded-lg ${
                      pathname === ele.href
                        ? "bg-sidebar-hover text-white"
                        : "text-[#cfdae2cc]"
                    }`}
    ...
    return (
      <li
        key={key}
        className={`p-2 hover:bg-sidebar-hover w-full rounded-lg ${
          pathname === element.href && "bg-sidebar-hover"
        }`}
      >
        <Link
          onClick={() => {
  ...
  return (
    <>
      <>
        <nav
          className="sticky top-0 block  sm:hidden md:hidden lg:hiddenfixed z-50 w-full bg-sidebarbg"
          ref={divSideRef}
        >
                  ...
                  <div className="relative">
                    ...
                    {isOpen && (
                      <ul className="absolute w-[200px] text-center right-0 mt-2 py-2 bg-white rounded shadow-lg z-50">
                        ...
                        {state &&
                        state?.email?.split("@")[1] === "joinnextmed.com" ? (
                          <>
                            {SideBarRoutes.map(
                              (element, key) =>
                        ...
                        <li className="p-2 relative hover:underline flex">
                          ...
                          />
                          <label
                            className="inline-block  rounded-full  text-heading  justify-between  ml-4 whitespace-nowrap text-sm font-normal "
                            for="file_input"
                          >
                            Upload logo
                          </label>
              ...
              >
                {skeltonLoading ? (
                  ...
                  <ul className="font-medium p-2 relative  bg-sidebarroute rounded-lg transition-all duration-300 ease-in-out">
                    <li
                      className="p-2 group hover:bg-sidebarsubroute flex  gap-2 items-center rounded-lg cursor-pointer"
                      onClick={() => handleToggle()}
                    >
                      {state?.enterprise?.logo ? (
                        <img
                        ...
                        <div className="relative inline-flex items-center justify-center w-10 h-10 overflow-hidden bg-[#E3AC2D] rounded-lg dark:bg-gray-600">
                          <span className="font-medium text-white normal-case">
                            {" "}
                            {state?.enterprise?.name.charAt(0)}
                          </span>
                        </div>
                      )}
                      <div className="relative ">
                        ...
                        <p className="text-[12px] text-normal">
                          {makeCapital(state?.role)}
                        </p>
                      </div>
                    </li>

                    <li
                      className="p-2 hover:bg-sidebar-hover w-full rounded-lg  cursor-pointer"
        ...
        <aside
          id="logo-sidebar"
          className={`hidden sm:block fixed py-5 bg-sidebarbg  text-white top-0 left-0 z-40 ${
            collaps
        ...
        >
          <div className="h-full px-2 pb-4 overflow-y-auto bg-sidebarbg  text-white">
                    ...
                    <li className="group mb-2 items-center rounded-lg cursor-pointer">
                      ...
                      >
                        <div className="flex items-center justify-start">
                          ...
                          <div
                            className={`flex justify-between gap-1 items-center w-full p-[0.75rem] rounded-lg`}
                          >
                            <img
                              className="w-6 h-6 rounded-lg"
                              src={"/tempologo.png"}
            ...
            >
              ...
              <div
                className={`cursor-pointer py-4 px-2 flex ${
                  !collaps ? "justify-start" : "justify-center"
                }`}
                onClick={(e) => {
              ...
              >
                ...
                {/* <li className="p-2 hover:bg-sidebar-hover w-full rounded-lg  cursor-pointer"> */}
                {!collaps ? (
                  <p className="px-2 text-[12px] font-normal flex items-center gap-2">
                    {" "}
        ...
        <div
          className={`${
            collaps
              ? "pt-2 sm:pl-20 transition-all duration-300 ease-in-out"
              : "pt-2 sm:pl-64 transition-all duration-300 ease-in-out"
          } bg-sidebarbg`}
        >
          <div className="bg-white p-4 rounded-tl-lg">{children}</div>
        </div>
      </>
    </>
  );
