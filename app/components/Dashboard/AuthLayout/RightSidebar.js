...
const RightSidebar = ({ children }) => {
    ...
    return (
        <>
            <nav
                id="sidenav-7"
                className="pt-[5rem] hidden sm:block md:block lg:block fixed right-0 top-0 h-screen w-60 translate-x-full overflow-hidden bg-white shadow-[0_4px_12px_0_rgba(0,0,0,0.07),_0_2px_4px_rgba(0,0,0,0.05)] data-[te-sidenav-hidden='false']:-translate-x-0 dark:bg-zinc-800 p-2"
                ...
                <ul className="relative m-0 list-none px-[0.2rem]">
                    ...
                    {list.map((ele, key) =>
                        <li className='my-4 cursor-pointer' key={key}>
                            <div className='flex justify-between items-center'>
                                <div className='flex justify-between items-center gap-2'>

                                   {ele.icon}<p className='text-heading text-sm'>{ele.name}</p></div>
                                <span><ChevronRightIcon className="h-5 w-5 text-gray-500" /></span>
                            </div></li>
                    )}
                </ul>
            </nav>
            <div className='w-[auto] sm:w-[80%] md:w-[80%] lg:w-[80%]'>
                {children}
            </div>
        </>
    )
