...
const SelectField = ({ labelClassName, title, values, id, register, error }) => {
    return (
        <div className={`inline ${labelClassName || ''}`}>
            ...
            <select
                className={`mt-1 block w-full px-3 py-1 bg-white border border-input_color rounded-md text-sm shadow-sm placeholder-slate-400  focus:outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500 disabled:bg-slate-50 disabled:text-slate-500 invalid:border-pink-500 invalid:text-pink-600 focus:invalid:border-pink-500 focus:invalid:ring-pink-500 ${error?.message && ("border-danger invalid:border-danger invalid:text-danger focus:invalid:border-danger focus:invalid:ring-danger focus:ring-danger")}`}
                id={id} name={id}
                {...register}>
