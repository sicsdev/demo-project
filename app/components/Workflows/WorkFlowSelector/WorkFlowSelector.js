...
const WorkFlowSelector = ({
  ...
  openModal,
  stepData,
  setAutomationStepsData,
  workflowId,
  indexSelector,
  setIndexSelector,
  setAddStepIndex,
  automationStepsField,
  setAutomationStepsField,
  getWorkflowData,
  singleData1,
  isAuthorizedUser,
}) => {
  ...
  const DisableButton = (index) => {
    ...
    if (
      automationStepsField[index].names_arr.length === 0 &&
      automationStepsField[index].output === ""
    ) {
      //
      return true;
    }
    if (
      !jsonPattern.test(automationStepsField[index].output) &&
      automationStepsField[index].output !== ""
    ) {
      return true;
    }
  ...
  const deleteTheEntry = async (key) => {
    ...
    if (singleData1?.active) {
      ...
      const update = await updateWorkFlowStatus(
        { active: false, automations: get_ids },
        workflowId
      );
  ...
  const handleAgentNameValue = (e, index) => {
    ...
    if (value.includes(",")) {
      ...
      agentNames.forEach((name) => {
        ...
        if (
          old_names &&
          !old_names.includes(makeCapital(trimmedName)) &&
          trimmedName
        ) {
  ...
  const SubmitFormValue = async (index) => {
    setLoading(index);
    const findUpdateValue = automationStepsField.find(
      (_, key) => key === index
    );
    ...
    const payload = {
      automations: [
        ...get_ids,
        {
          automation: findUpdateValue.key,
          data: convertArrayToObject(findUpdateValue.names_arr),
          output: outputObject,
        },
      ],
    };
  ...
  const removeIndexAndUpdateAutomation = async () => {
    ...
    filterRules = automationDragHandle.arr.filter(
      (x) => x.id !== automationDragHandle.automation
    );
    if (automationDragHandle.deflection !== null) {
      filterRules = filterRules.filter(
        (x) => x.id !== automationDragHandle.deflection
      );
    }
  ...
  const onDragEnd = (result) => {
    ...
    reorder(stepData, result.source.index, result.destination.index);
    setMainClass(null);
  };
  ...
  return (
    <>
            ...
            >
              <div className="w-[auto] xs:w-[80%] sm:w-[70%] md:w-[70%] lg:w-[70%] mx-auto">
                ...
                {stepData?.length === 0 && !isAuthorizedUser && (
                  <div
                    className={`mt-4 border-2 border-dashed  bg-[white] ${
                      indexSelector === null
                        ? "border-primary"
                        : "border-border"
                    } rounded-lg shadow p-5 cursor-pointer group`}
                    onClick={(e) =>
                      openModal({ key: "STEPS", open: true, addKey: 0 })
                    }
                  >
                    <div className="flex justify-between gap-2 items-center">
                      <div className="flex justify-between gap-4 items-center">
                ...
                <>
                  {stepData?.map((ele, key) => (
                    <Draggable
                      isDragDisabled={!isAuthorizedUser}
                      key={ele.id}
                      draggableId={ele.id}
                      index={key}
                    >
                      {(provided, snapshot) => (
                        ...
                        >
                          <>
                            {indexSelector === key && (
                              <div
                                className="relative mt-4 border-2 border-dashed  bg-[white] border-primary rounded-lg shadow p-5 cursor-pointer group"
                                onClick={(e) =>
                                  openModal({
                                    key: "STEPS",
                                    open: true,
                            ...
                            <div
                              className={`${mainClass === ele.id && "drag"}`}
                              onMouseEnter={() =>
                                updateShowButtonState(key, "show")
                              }
                              onMouseLeave={() =>
                                updateShowButtonState(key, "hide")
                              }
                            >
                              {key !== 0 && (
                                            ...
                                            <div
                                              className={`${
                                                showButtonStates == key
                                                  ? "bg-white"
                                                  : ""
                                              } rounded-lg group-hover:border border-border  group-hover:shadow] p-[2px]`}
                                            >
                                              {showButtonStates == key && (
                                                <>
                                                  <Button
      ...
      {modal === true && (
        <Modal
          title={"Are you sure you want to move this automation? "}
          show={modal}
          setShow={setModal}
          showCancel={true}
          className={
            "w-[100%] sm:w-[50%] md:w-[50%] lg:w-[50%] my-6 mx-auto sm:max-w-[50%] md:max-w-[50%] lg:max-w-[50%]"
          }
        >
          <>
            <div className="">
              <p className="text-heading font-normal text-normal">
                {" "}
                Your rules will be deleted and you'll have to reenter them after
                moving this.
              </p>
            </div>
            <div className="">
