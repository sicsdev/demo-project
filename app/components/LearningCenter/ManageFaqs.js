...
import {
  ...
  getFaqHistory,
  getFaqQuestionById,
  patchKnowledgeQuestion,
} from "@/app/API/pages/Knowledge";
import {
  addNagetiveQuestionData,
  addNegativeBulkCreate,
  deleteNagetiveQuestionData,
  editNagetiveQuestionData,
  getNagetiveQuestionData,
  getSingleNagetiveQuestionData,
} from "@/app/API/pages/NagetiveFaq";
...
import PromptModifiersTab from "./PromptModifiersTab/PromptModifiersTab";
...
const ManageFaqs = ({
  ...
  const [externalTitleForSnippet, setExternalTitleForSnippet] =
    useState("Products");
  ...
  useEffect(() => {
    ...
    if (openedKnowledge) {
      handleAutoOpenKnowledge(openedKnowledge);
    }
  }, []);
  ...
  const updateBotSelection = async (rowIndex, selectedBots) => {
    ...
    const payload = {
      bots: selectedBots.map((botId) => ({ bot: botId.value, active: true })),
    };
    try {
  ...
  const getIndex = (elementId) => {
    const findIndex = questions?.data?.results.findIndex(
      (x) => x.id === elementId
    );
    return findIndex;
  };
  ...
  const addNewNagetiveFaq = async () => {
    ...
    if (isEdit === false) {
      ...
      if (bulkCreate?.data?.length > 0) {
        setNagetiveQuestions(bulkCreate?.data);
      }
      cleanTextArea();
    } else {
      let edit = await editNagetiveQuestionData(
        { search: selected.negative_answer },
  ...
  const columns = [
    {
      ...
      cell: (row) => (
        <p
          className="whitespace-normal p-2"
          onClick={() => {
            setSelected({
              selectBots: row?.bots.map((x) => {
                return { name: x.bot.chat_title, value: x.bot.id };
              }),
              ...row,
            });
          }}
    ...
    },
    {
      ...
      cell: (row) => (
        <div
          className="flex justify-start w-full items-center gap-2 ml-[7px] text-[#626060]"
          onClick={() => {
            setSelected({
              selectBots: row?.bots.map((x) => {
                return { name: x.bot.chat_title, value: x.bot.id };
    ...
    },
    {
      ...
      cell: (row) => (
        <p
          className="whitespace-normal p-2 ml-[35px] text-[#626060]"
          onClick={() => {
            setSelected({
              selectBots: row?.bots.map((x) => {
                return { name: x.bot.chat_title, value: x.bot.id };
        ...
        >
          {parseInt(row.knowledgefaq_usage_last_24_hours).toLocaleString()}
        </p>
      ),
    },
    {
      ...
      id: "bots",
      cell: (row, index) => (
        <div className="py-2 searchWrap overflow-x-scroll !w-[100%]">
          <Multiselect
            className="w-[200px]"
            options={bots ?? []}
            selectedValues={
              questions.selectedBot ? questions?.selectedBot[index] : []
            }
            ...
            placeholder={
              questions?.selectedBot &&
              questions?.selectedBot[index]?.length === questions?.bots?.length
                ? ""
                : "Select Bots"
            }
            displayValue="name"
      ...
      return (
        <div className="mt-[15px] md:mt-0 flex gap-6 items-center">
          <div className="flex justify-start w-full items-center gap-3">
            ...
            <h2 className={`text-heading text-sm !font-semibold opacity-90`}>
              <span className="text-sm text-primary font-semibold">
                {" "}
                Usage:
              </span>{" "}
              {selected.knowledgefaq_usage_last_24_hours}
            </h2>
          </div>
        </div>
      );
      ...
      <div className="knowledgebase_table  px-2 pt-2 w-[98%] knowledge">
        <div className=" hidden sm:block md:block lg:block">
          <DataTable
            ...
            pagination
            className={`${
              questions?.data?.results?.length <= 5
                ? "h-[300px] sm:h-[400px] overflow-y-scroll"
                : "!h-[117vh] !overflow-y-hidden !overflow-x-hidden  "
            } myDataTable`}
            columns={currentTab == "products" ? productColumnsInfo : columns}
            noDataComponent={
              <>
                <p className="text-center text-xs p-3">
                  Answers to questions your customers may have will show here
                  when you add them.
                </p>
              </>
            }
            ...
            onRowClicked={(rowData) => {
              currentTab == "products"
                ? handleOpenEditProduct(rowData)
                : setSelected({
                    selectBots: rowData?.bots.map((x) => {
                      return { name: x.bot.chat_title, value: x.bot.id };
                    }),
                    ...rowData,
                  });
        ...
        <div className=" block sm:hidden md:hidden lg:hidden">
          <DataTable
            ...
            noDataComponent={
              <>
                <p className="text-center text-xs p-3">
                  Questions Deflection AI needs your help answering will show
        ...
        {selected && (
          ...
          >
            <div
              className={
                "border-b-2 my-2 border-border dark:border-gray-700 flex items-center justify-between w-[98.5%]"
              }
            >
              ...
              <>
                ...
                <div className="py-2 w-[97%] m-auto">
                  <Multiselect
                    ...
                    options={bots}
                    selectedValues={
                      selected?.selectBots ? selected?.selectBots : []
                    }
                    onSelect={(selectedList, selectedItem) => {
                      setSelected((prev) => {
