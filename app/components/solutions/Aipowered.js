...
const Aipowered = () => {
  const data = [
    ...
    {
      title: "How Heroes saves two working days a month using Bob’s automated workflows and integrations",
      desc:"Heroes is a tech startup based in London, with additional offices in Madrid and Barcelona,",
      logo: "/2.webp",
      link: "",
    },
    {
      title: "First impressions count! How Bob creates personalized onboarding experiences new hires won’t forget.",
        ...
        <div className="hidden sm:grid max-w-[90%] m-auto mt-4 grid-cols-3 gap-4 sm:grid-cols-3 text-center sm:gap-8">
          {data.map((element, key) => (
            <div className="shadow-md  pb-[101px]   relative">
              <div
                className="relative w-[100%]  hover:text-white "
                key={key}
              >
                <img
                  src={element.logo}
...
<ArrowLongRightIcon className="h-10 w-10 text-gray-500" />               </a>
            </div>
          ))}
        </div>
      </div>
