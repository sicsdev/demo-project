              ...
              <Link href="/">
                    ...
                    <ul className='hidden relative md:flex text-white gap-8 flex-row'>
                        {nav_links.map((element, key) =>
                            <li key={key} className='py-[15px] h-[60px] group relative cursor-pointer hover:border hover:border-b-white hover:border-r-0 hover:border-l-0 hover:border-t-0 '><Link href={element.link}>{element.name}</Link>
                                {element.card.links.length > 0 && (
                                    <Card className={'animate-fadeIn w-[800px] hidden group-hover:block absolute top-[61px] bg-white'}>
                                        <List className={'grid grid-cols-2 gap-8'} nav_links={element.card.links} />
                                    </Card>
                                )}
                            </li>
                        )}
