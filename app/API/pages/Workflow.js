...
export const getWorkflowByStatus = async (status, source, bot) => {
    let config = returnConfig()
    try {
        const response = await axios.get(`${API_URL}/api/v1/main/workflows?active=${status}${source ? `&source=${source}` : ''}${bot ? `&bot=${bot}` : ''}&ordering=-annotated_successful_automation_usage_last_24_hours_count`, config);
        return response.data;
    } catch (error) {
        return error
    }
