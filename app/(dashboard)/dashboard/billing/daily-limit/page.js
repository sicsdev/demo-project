...
const UsageLimit = () => {
  ...
  const SubmitForm = async () => {
    setBtnLoading(true)
    const response = await updateThresholds({ "billing_thresholds": { "amount_gte": parseInt(formData) } })
      ...
      {loading === true ? <Loading /> :
        <div>
            ...
            <ul className="flex flex-wrap -mb-px text-sm font-medium text-center text-gray-500 dark:text-gray-400">
              <li className="mr-2">
                <a href="#" className=" flex justify-start gap-2 items-center p-4 text-primary font-bold border-b-2 border-primary rounded-t-lg active  group" aria-current="page">
                  <CurrencyDollarIcon className="h-6 w-6 text-gray-500" /> Billing Threshold
                </a>
              </li>
          ...
          <div className='w-full sm:w-[60%] md:w-[60%] lg:w-[60%] mx-auto my-5'>
            <h3 className="font-bold text-xl md:text-xl lg:text-xl sm:text-xl sm:leading-none my-2 text-heading">Billing Threshold</h3>
            <p className='text-sm my-2'>Your payment method on file will be charged each time your usage hits your threshold. Your threshold is adjusted automatically based on your usage. You can also edit it here.
            </p>
            <TextField
            ...
            <p className='text-sm mt-1'>Your total usage so far in July (UTC). Note that this may include usage covered by a free trial or other credits,
              so your monthly bill might be less than the value shown here. <Link className='text-primary hover:text-border font-medium' href={"/dashboard/billing/usage"}>View usage records</Link></p>
            <p className='text-lg'>${totalUsage}</p>
