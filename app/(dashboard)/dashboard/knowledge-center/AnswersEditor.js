                                            ...
                                            <div className={`my-4 rounded-md p-3 px-2`}>
                                                <ul className="text-start py-2 text-sm text-gray-700" style={{ maxHeight: '61vh', overflowY: 'scroll' }}>
                                                    ...
                                                    {subQuestions?.map((element, key) =>
                                                        <li className='p-4 text-justify bg-[#96b2ed2e] text-heading my-2 cursor-pointer mr-3' key={key}>
                ...
                {tab === 1 && (
                    <div>
                        <div className=' mt-2 '>
                            ...
                            {workFlowData.reccomodation.length > 0 &&
                                <div className='flex justify-end'>
                                    <Button
                                        type={"button"}
                                        className={`mr-5 inline-block rounded bg-primary px-2 pt-2 mt-3 text-xs font-medium leading-normal text-white disabled:shadow-none  transition duration-150 ease-in-out hover:bg-success-600 hover:shadow-[0_8px_9px_-4px_#0000ff8a] focus:bg-success-600 focus:shadow-[0_8px_9px_-4px_#0000ff8a] focus:outline-none focus:ring-0 active:bg-success-700 active:shadow-[0_8px_9px_-4px_#0000ff8a]`}
                                        onClick={() => setShowRecommendedWorkflows(!showRecommendedWorkflows)}

                                    >{!showRecommendedWorkflows ? "Show recommendations" : "Show workflows list"}</Button>
                                </div>
                            }
                        ...
                        {!showRecommendedWorkflows &&
                            <>
                                <div>
                                    <h1 className="text-xs font-semibold mx-1 mb-2 mt-5">Select an Existing Workflow:</h1>
                                </div>
